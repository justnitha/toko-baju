...
const WingtipCrossWhite = () => {
  const [Title, SetTitle] = useState("T-Shirt Kids Cross Font White -");
  ...
  const CustomPrevArrow = (props) => (
    <button
      className={`absolute top-[17rem] left-2 cursor-pointer z-10 ${isModalOpen ? "hidden" : "" } `}
  ...
  const CustomNextArrow = (props) => (
    <button
      className={`absolute right-2 bottom-[16rem] ${isModalOpen ? "hidden" : "" }`}
      onClick={props.onClick}
    >
  ...
  const settings = {
    ...
    customPaging: function (i) {
      return (
        ...
        >
          <img
            src={require(`../../plugin/img/ccloth/cloth-7-${i + 1}.png`)}
  ...
  return (
    <div className={`${isModalOpen ? "" : ""}`}>
      <NavbarBagi />
      <div className="grid grid-cols-2 gap-5 w-[80%] mx-auto mt-28 z-50">
        <Slider {...settings}>
          {Cloth7.map((item, index) => (
            <div
              key={index}
      ...
      <Modal7 isOpen={isModalOpen} onClose={handleCloseModal} />
    </div>
  );
};
